...
class Cnn_9layers_Gru_FrameAtt(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin,
        ...
        self.s3prl_feature_type = 'spectrogram'
        self.extractor_s3prl = torch.hub.load('s3prl/s3prl', self.s3prl_feature_type)
...
#        freeze_parameters=True)
    ...
    def forward(self, x, mixup_lambda=None, timeshift=False):
        """Input: (batch_size, times_steps, freq_bins)"""
        ...
        print(f"\n{x.shape}")
        print(x)
        wavs = [wav for wav in x.squeeze(1)]
        print(f"\n\n wavs {len(wavs)}")
        print(wavs)
        ...
        x = self.bn0(x)
        x = x.transpose(1, 3)
        print(f"After transpose 2:\n{x}")

        if self.training:
            print(f"Before spec_augmenter:\n{x}")
            x = self.spec_augmenter(x)
        ...
        if self.training and mixup_lambda is not None and not timeshift:
            print(f"Before do_mixup :\n{x}")
            x = do_mixup(x, mixup_lambda)
            print(f"After do_mixup :\n{x}")


        if self.training and mixup_lambda is not None and timeshift:
            print(f"Before do_mixup_timeshift :\n{x}")
            x = do_mixup_timeshift(x, mixup_lambda)
            print(f"After do_mixup_timeshift :\n{x}")

        print(f"Before conv input :\n{x}")
        x = self.conv_block1(x, pool_size=(2, 2), pool_type='avg')
        x = self.conv_block2(x, pool_size=(2, 2), pool_type='avg')
        x = self.conv_block3(x, pool_size=(2, 2), pool_type='avg')
        x = self.conv_block4(x, pool_size=(1, 1), pool_type='avg')
        ...
        if framewise_output.size()[1] != 1000:
            framewise_output = pad_framewise_output(framewise_output, framewise_output.size()[1])
